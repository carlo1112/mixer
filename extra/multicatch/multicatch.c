...
t_int *mcatch_tilde_perform(t_int *w)
{
   ...
   *(x->x_executed) = *(x->x_executed) + 1;
    ...
    {
        *out++ = *in;
        if( *(x->x_executed)==*(x->x_total_instances) ) //si es la ultima instancias pone en cero el buffer del mcatch
            *in++ = 0;
        else
            *in++;
    }
...
void mcatch_tilde_free(t_mcatch_tilde *x)       //borra la instancia de mcatch
{
    *(x->x_total_instances) = *(x->x_total_instances) - 1;      //disminuye la cantidad de instancias
    ...
    if(*(x->x_total_instances)==0)//all gone
    {
        ...
        // t_mcatch_tilde *x_zero = (t_mcatch_tilde *) mct_zero[x->mct_idx];

        // freebytes(x_zero->x_vec, x->x_n * sizeof(t_sample));
        // freebytes(x_zero->x_total_instances, sizeof(int) );
        // freebytes(x_zero->x_executed, sizeof(int) );
        // freebytes(x_zero, sizeof(t_mcatch_tilde) );
    }
    else
    {
